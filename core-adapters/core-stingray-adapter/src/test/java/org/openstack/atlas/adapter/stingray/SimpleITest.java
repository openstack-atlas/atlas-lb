...
import com.riverbed.stingray.service.client.CatalogMonitorType;
import com.riverbed.stingray.service.client.PoolLoadBalancingAlgorithm;
import com.riverbed.stingray.service.client.PoolWeightingsDefinition;
...
public class SimpleITest extends ITestBase {
    ...
    private void createNodes() throws Exception {
        ...
        Node node3 = new Node();
        Node node4 = new Node();
        node3.setAddress("192.168.37.20");
        node4.setAddress("192.168.37.21");
        node3.setPort(80);
        node4.setPort(80);
        ...
        String node3StingrayString = IpHelper.createStingrayIpString(node3.getAddress(), node3.getPort());
        String node4StingrayString = IpHelper.createStingrayIpString(node4.getAddress(), node4.getPort());
        ...
        Assert.assertEquals(node1StingrayString, enabledNodes[0][0]);
        Assert.assertEquals(node3StingrayString, enabledNodes[0][1]);
        ...
        Assert.assertEquals(node2StingrayString, disabledNodes[0][0]);
        Assert.assertEquals(node4StingrayString, disabledNodes[0][1]);
        ...
        for (PoolWeightingsDefinition weightingsDefinition : weightingsDefinitions[0]) {
            if (weightingsDefinition.getNode().equals(node1StingrayString))
                Assert.assertEquals(defaultNodeWeight, weightingsDefinition.getWeighting());
            else if (weightingsDefinition.getNode().equals(node2StingrayString))
                Assert.assertEquals(defaultNodeWeight, weightingsDefinition.getWeighting());
            else if (weightingsDefinition.getNode().equals(node3StingrayString))
                Assert.assertEquals(node3.getWeight().intValue(), weightingsDefinition.getWeighting());
            else if (weightingsDefinition.getNode().equals(node4StingrayString))
